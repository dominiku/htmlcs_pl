...
_global.HTMLCS_WCAG2AAA_Sniffs_Principle1_Guideline1_3_1_3_1 = {
	...
	testPresentationMarkup: function(top)
	{
		...
		if (doctype && (doctype === 'html5' || doctype === 'xhtml5')) {
			...
			for (var i = 0; i < tags.length; i++) {
				...
				HTMLCS.addMessage(HTMLCS.ERROR, tags[i], _global.translation["1_3_1_H49.AlignAttr"], msgCode);
			}
		} else {
	...
	testTableHeaders: function(table)
	{
		...
		for (var i = 0; i < headersAttr.wrongHeaders.length; i++) {
			HTMLCS.addMessage(HTMLCS.ERROR, headersAttr.wrongHeaders[i].element,
				_global.translation["1_3_1_H43.IncorrectAttr"].replace(/\{\{expected\}\}/g, headersAttr.wrongHeaders[i].expected).replace(/\{\{actual\}\}/g, headersAttr.wrongHeaders[i].actual), 'H43.IncorrectAttr');
		}
